...
<script type="text/javascript">
	...
	$(function(){
			...
			$.ajax({
				...
				success : function (data) {
					...
					$.each(data, function (index, content) {
						$("#create-owner").append("<option value='" + content.id + "'>" + content.name + "</option>")
					})
		...
		//	为头部复选框绑定单击事件
		$("#clueTblHead").click(function () {
			$("input[name=select]").prop("checked", this.checked);
		})

		//	非全选
		$("#clueTblBody").on("click", $("input[name=select]"), function () {
			$("#clueTblHead").prop("checked", $("input[name=select]").length === $("input[name=select]:checked").length);
		})
	...
	function pageList(pageNo, pageSize) {
		...
		$.ajax({
			...
			success : function (data) {
				/*
				* data	["total":count, "clueList":[{clue1},{clue2},...,{cluen}]]
				* */
				...
				<table class="table table-hover">
					<thead>
						<tr style="color: #B3B3B3;">
							<td><input type="checkbox" id="clueTblHead"/></td>
							<td>名称</td>
							<td>公司</td>
							<td>公司座机</td>
							<td>手机</td>
